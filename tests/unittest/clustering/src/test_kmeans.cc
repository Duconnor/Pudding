...
TEST_CASE ("Test GPU kmeans using the CPU version using a large number of examples", "[kmeans-cpu-gpu-large]") {
    std::srand(0);
...
TEST_CASE ("Test GPU kmeans when the number of centers exceeds the largest possible block width (i.e. 1024)", "[kmeans-gpu-large-number-centers]") {
    std::srand(0);
